...
from XKTools import FileOperations
import ZhiHuDao
...
def analysis_all_domains_nearly_today(content_json, save_path):
    ...
    FileOperations.writeFile(save_path, 'domains_' + time_str + '.txt', str(content_json))
    bean_list = []
...
def get_all_week_hot_questions(main_url, save_path, d_c0_x, domain_id, name):
    ...
    while True:
        try:
            ...
            FileOperations.writeFile(save_path, name + '_' + str(remember_count) + '.txt', json_str)
...
def analysis_all_week_hot_questions(save_path):
    bean_list = []
    for file_name in FileOperations.getFolderFileNames(save_path):
        print(file_name)
        source_stype = file_name.split('_')[0]
        file_content = json.loads(FileOperations.readFile(save_path, file_name))
    ...
    for item in ZhiHuDao.select_zhihu_questions():
        ...
        while True:
            try:
                ...
                FileOperations.writeFile(save_path, question_token + '_' + str(page) + '.txt', json_str)
...
def analysis_all_question_details(save_path):
    file_name_list = FileOperations.getFolderFileNames(save_path)
    for file_name in file_name_list:
        main_json_main = FileOperations.readFile(save_path, file_name)
        domain_token = file_name.split('_')[0]
        main_json_main = json.loads(main_json_main)
        for item in main_json_main['data']:
            question_token = item['id']
            question_url = item['url']
            question_title = item['title']
            question_content = item['content']
            question_author = item['author']['name']
            question_author_url = item['author']['url']
            question_author_avatar = item['author']['avatar_url']
            question_author_is_following = item['author']['is_following']
            question_author_follower_count = item['author']['follower_count']
...
if __name__ == '__main__':
    ...
    FileOperations.createFolder(save_path_questinions_details)
